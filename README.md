...
| `mysql_health_check` | Run health checks on a MySQL instance and return key metrics with status (`ok` / `warning` / `critical`). Checks: InnoDB buffer pool hit rate, buffer pool size vs available RAM, thread cache hit rate, temporary tables on disk, InnoDB history list length, max connections usage, flushing logs ratio (ok < 5%), sort merge passes ratio (ok < 10%), and InnoDB redo log fill time (optimal: 45–75 min) |
